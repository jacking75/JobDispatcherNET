...
///
/// 코딩 컨벤션 (GameZone과 동일):
///   public X(args)         → DoAsync(() => ProcessX(args))   (큐에 푸시만)
///   private ProcessX(args) → 실제 본문 (디버거/스택트레이스/grep 친화적)
/// </summary>
public class PlayerActor : AsyncExecutable
{
    ...
    private static readonly TimeSpan RespawnDelay = TimeSpan.FromSeconds(5);
    ...
    public void ReceiveAreaDamage(AttackerSnapshot attacker, float centerX, float centerY, float radius)
        => DoAsync(() => ProcessReceiveAreaDamage(attacker, centerX, centerY, radius));
    ...
    /// </summary>
    public void Despawn() => DoAsync(ProcessDespawn);

    // ── 실제 본문 (private, PlayerActor 큐에서 직렬 실행) ─────────────
    ...
    private void ProcessReceiveAreaDamage(AttackerSnapshot attacker, float centerX, float centerY, float radius)
    {
        if (_despawned || !_player.IsAlive) return;
    ...
    /// <summary>
    /// HandleDeath는 ProcessReceive* 안에서 (= 자기 큐 안에서) 호출되므로
    /// DoAsync 가 아닌 직접 호출. 부활은 5초 뒤 자기 큐에 예약한다.
    /// </summary>
